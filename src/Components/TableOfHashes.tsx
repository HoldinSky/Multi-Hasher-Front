...
import parseDataToVisible from "../common/parseDataToVisible";
...
interface Props {
    finishedTasks: IHashResults[];
    clearHistoryHandler: () => void;
}

let dataToRender: DataToRender[] = [];

function TableOfHashes(props: Props) {
    ...
    useEffect(() => {
        dataToRender = parseDataToVisible(results);
        const numEmptyRows =
            page > 0 ? Math.max(0, (1 + page) * rowsPerPage - dataToRender.length) : rowsPerPage - dataToRender.length;
    ...
    const handleChangePage = useCallback(
        (event: unknown, newPage: number) => {
            ...
            const numEmptyRows =
                newPage > 0 ? Math.max(0, (1 + newPage) * rowsPerPage - dataToRender.length) : rowsPerPage - dataToRender.length;
            ...
            <Paper sx={{width: '100%', mb: 2}}>
                ...
                <TableContainer>
                    ...
                    >
                        ...
                        <TableBody>
                            {dataToRender
                                ? dataToRender.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((value, index) => {
                                    return (
                                        <TableRow
                                            hover
                                            tabIndex={-1}
                                        ...
                                        >
                                            ...
                                            <TableCell align="right"><Link to={"/details"} state={{ hashResult: results[index], }}>{value.result}</Link></TableCell>
                                        </TableRow>
                                    );
                                })
                                : null}
